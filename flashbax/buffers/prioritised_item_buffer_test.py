...
from flashbax.buffers import prioritised_item_buffer, sum_tree
...
def test_add_and_can_sample(
    ...
    buffer = prioritised_item_buffer.make_prioritised_item_buffer(
        max_length, min_length, 4, False, True
    )
...
def test_sample(
    ...
    # Check that the transitions have been updated.
    with pytest.raises(AssertionError):
        chex.assert_trees_all_close(batch1.experience, batch2.experience)
...
def test_adjust_priorities(
    ...
    # Fill buffer to the point that we can sample.
    fake_batch = get_fake_batch(fake_transition, add_batch_size)
    buffer = buffer = prioritised_item_buffer.make_prioritised_item_buffer(
        max_length, min_length, sample_batch_size, False, True, priority_exponent
    )
    state = buffer.init(fake_transition)
...
def test_add_batch_size_none(
    ...
    buffer = prioritised_item_buffer.make_prioritised_item_buffer(
        max_length, min_length, 4, False, False
    )
    state = buffer.init(fake_transition)
